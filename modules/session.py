...
_COACH_TIPS = {
    ...
    "prepare": [
        {"icon": "🧠",
         "en": "**Prepare is your warm-up.** The goal is to familiarise your brain with the material before real-time shadowing — where there's no time to think or decode meaning. If you can understand the text and follow the audio comfortably, you're ready to shadow.",
         "zh": "热身阶段的目的，是让大脑提前熟悉练习材料。因为在实时跟读时，根本没有时间思考和理解文本。如果这个阶段理解无障碍、听力无障碍，你就可以进入下一步的跟读了。"},
        {"icon": "📖",
        ...
        {"icon": "🐢",
         "en": "**Struggling with fast speech?** Slow it down. Click the ··· options button on the far right of the audio player to adjust playback speed.",
         "zh": "遇到语速太快听不清的句子，可以调慢速听。点击音频条最右边的更多选项（···）可以调整播放速度。"},
        {"icon": "🔤",
         "en": "**Annotations mark how native speech connects.** The symbols show where speakers compress and link sounds. Studying them helps you internalise these connected-speech patterns.",
         "zh": "语音符号标注的是母语者压缩和连接语流的发音特征。了解这些特征，有助于你内化英语语流现象。",
         "link_phonology": True},
        {"icon": "🗣️",
...
def _phase3_guide():
    ...
    st.markdown(
        ...
        '蓝牙有1–2秒延迟，等麦克风显示 Ready 后再录音。</span><br>'

        '<strong>🔤 Sentence-by-sentence</strong>'
        '<span style="color:#6B7280;">: audio pauses after each sentence. '
        'Shadow <em>while the audio plays</em>. Use ← → to replay or move between sentences. '
        'Good for detailed practice. / '
        '每句播放完后暂停，跟读时与音频同步，可逐句回听或前后切换，适合精细练习。</span><br>'

        '<strong>🎵 Flow</strong>'
        '<span style="color:#6B7280;">: full passage plays continuously — shadow along without stopping. '
        'Good when you\'re already familiar with the material. / '
        '全段连续播放，不暂停，适合对材料已熟悉后的整体跟读。</span>'
...
def _phase_shadow():
    ...
    _phase_header(3, "Shadow", "跟读",
                  "#7C3AED", "#F5F3FF",
                  "Sentence-by-sentence or full-passage shadowing · 逐句精练 或 整段跟读")
    ...
    with _tv1:
        if st.button("👁 Hide text / 隐藏" if show_text else "👁 Show text / 显示文本",
                     key="p3_txt_toggle"):
            new_show = not show_text
            st.session_state.shadow_show_text = new_show
            if not new_show:
                st.session_state.shadow_show_ann = False
            st.rerun()
    with _tv2:
        if show_text:
            if st.button("🏷 Hide annotations / 隐藏注释" if show_ann else "🏷 Show annotations / 显示注释",
                         key="p3_ann_toggle"):
